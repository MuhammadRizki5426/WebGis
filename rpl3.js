...
function handleLogin() {
    ...
    if (users[username] && users[username].password === password) {
        ...
        showNotification(`Login berhasil sebagai ${userRole === 'admin' ? 'Admin' : 'User Biasa'}`, 'success');
    } else {
...
function updateUIAfterLogin() {
    ...
    if (document.getElementById('home').classList.contains('active')) {
        ...
        if (userRoleSpan) userRoleSpan.textContent = `Logged in as ${userRole === 'admin' ? 'Admin' : 'User'}`;
    }
...
function initMap() {
    if (!map) {
        map = L.map('map').setView([-0.9000, 119.8833], 13);
...
function addMarkerToMap(bengkel) {
    ...
    const marker = L.marker([bengkel.lat, bengkel.lng])
        .bindPopup(`
            <div style="min-width: 250px;">
                <h3 style="margin: 0 0 10px 0; color: #2c3e50;">${bengkel.name}</h3>
                <p style="margin: 0 0 8px 0; color: #666;">${bengkel.address}</p>
                <p style="margin: 0 0 8px 0;"><strong>Jenis:</strong> ${getTypeName(bengkel.type)}</p>
                <p style="margin: 0 0 8px 0;"><strong>Area:</strong> ${getAreaName(bengkel.area)}</p>
                <p style="margin: 0 0 8px 0;"><strong>Rating:</strong> ${bengkel.rating} <i class="fas fa-star" style="color: gold;"></i></p>
                ${actionButtons}
            </div>
        `);
...
function getTypeName(type) {
    switch(type) {
        case 'motor': return 'Bengkel Motor';
        case 'mobil': return 'Bengkel Mobil';
        case 'umum': return 'Bengkel Umum';
...
function getAreaName(area) {
    switch(area) {
        case 'barat': return 'Palu Barat';
        case 'timur': return 'Palu Timur';
        case 'utara': return 'Palu Utara';
        case 'selatan': return 'Palu Selatan';
        default: return 'Tidak Diketahui';
    }
}
...
function filterBengkel() {
    ...
    const filteredBengkel = bengkelData.filter(bengkel => {
        ...
        if (searchText !== '') {
            ...
            const typeNameMatch = getTypeName(bengkel.type).toLowerCase().includes(searchText);
            const areaNameMatch = getAreaName(bengkel.area).toLowerCase().includes(searchText);
 ...
 */
function renderBengkelList(bengkelList = bengkelData, containerId = 'bengkelList') {
    ...
    bengkelList.forEach(bengkel => {
        ...
        const actionButtons = userRole === 'admin' ? `
            <div class="bengkel-actions">
                <button class="btn-edit" onclick="openEditModal(${JSON.stringify(bengkel).replace(/"/g, '&quot;')})">
                    <i class="fas fa-edit"></i> Edit
                </button>
                <button class="btn-delete" onclick="deleteBengkelFromMap(${bengkel.id})">
                    <i class="fas fa-trash"></i> Hapus
                </button>
            </div>
        ` : '';
        ...
        item.innerHTML = `
            ...
            <div class="bengkel-meta">
                <span class="bengkel-type ${bengkel.type}">${getTypeName(bengkel.type)}</span>
                <span class="bengkel-area">${getAreaName(bengkel.area)}</span>
            </div>
            <div class="bengkel-rating">
                <span class="bengkel-rating-stars">${createRatingStars(bengkel.rating)}</span>
...
function setupEventListeners() {
    ...
    if (homeSearchBtn && homeSearchInput) {
        homeSearchBtn.addEventListener('click', function(e) {
            ...
            if (searchText) {
                const petaLink = document.querySelector('.nav-link[data-target="peta"]');
                if (petaLink) {
                    petaLink.click();
                    ...
                    setTimeout(() => {
                        ...
                        if (petaSearchInput) {
                            ...
                            if (bengkelList && toggleList) {
                                ...
                                toggleList.innerHTML = '<i class="fas fa-times"></i> Tutup Daftar';
...
async function migrateLocalDataToSupabase() {
    ...
    try {
        ...
        // Jika belum ada data, migrasikan data lokal
        if (!existingData || existingData.length === 0) {
            console.log('Migrating local data to Supabase...');
            
            for (const bengkel of defaultBengkelData) {
                const { error } = await supabase
                    .from('bengkel')
                    .insert([bengkel]);
                
                if (error) {
                    console.error('Error migrating:', bengkel.name, error);
                }
            }
